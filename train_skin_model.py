...
DEFAULT_EVAL_DIR = Path("data") / "test"   # change to data/val if that's what you have
...
def load_model(device: str):
    ...
    if not meta_path.exists():
        raise FileNotFoundError("Missing artifacts/model_meta.json")
    if not classes_path.exists():
        raise FileNotFoundError("Missing artifacts/class_names.json")
    if not weights_path.exists():
        raise FileNotFoundError("Missing artifacts/skinsight_model.pt")
...
def main():
    ...
    # ⚠️ class order must match training order (class_names.json)
    # ImageFolder orders alphabetically. We must ensure ds.classes == class_names.
    if ds.classes != class_names:
        ...
        print("\nFix options:")
        print("1) Rename class folders so alphabetical order matches training order")
        print("2) Or rebuild your test folder to match class_names.json exactly")
        print("3) Or retrain and regenerate class_names.json from ImageFolder that matches your data\n")
        # Still continue, but metrics will be wrong if mismatched.
